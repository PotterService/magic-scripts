...
document.addEventListener("DOMContentLoaded", () => {
  state.fixedSet = code;
  setTimeout(() => {
    const s = MTG.sets.find(x=>x.code===code);
    const cards = MTG.cardsForSet(code);
    if(s){
      setCode.textContent = s.code.toUpperCase();
      setName.textContent = s.name;
      const owned=ownedCardCount(cards), pct=cards.length?Math.round((owned/cards.length)*100):0;
      setStats.textContent = `${owned} / ${cards.length} owned • ${pct}% complete`;
      updateTopStats(cards);
    }
    render();
  }, 50);
});
...
async function redownloadCurrentSet() {
  if (!code) return;
  ...
  try {
    ...
    const cards = MTG.cardsForSet(code);
    if (s) {
      setStats.textContent = `${ownedCardCount(cards)} / ${cards.length} owned • ${cards.length ? Math.round((ownedCardCount(cards)/cards.length)*100) : 0}% complete`;
    }
    updateTopStats(cards);
    render();
    Util.toast("Full set redownloaded");
  } catch(e) {
...
document.addEventListener("DOMContentLoaded", () => {
  setTimeout(() => {
    const btn = document.getElementById("redownloadSetBtn");
    if (btn) btn.onclick = redownloadCurrentSet;
  }, 100);
});
